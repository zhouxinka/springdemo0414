...
public class Page<T> {
     ...
     * @param request 传递 repage 参数，来记住页码
     * @param response 用于设置 Cookie，记住页码
     * dt-1:datatable;2:easyui
     */
    public Page(int dt, HttpServletRequest request, HttpServletResponse response){
        this(dt, request, response, -2);
    }
     ...
     * @param defaultPageSize 默认分页大小，如果传递 -1 则为不分页，返回所有数据
     */
    public Page(int dt ,HttpServletRequest request, HttpServletResponse response, int defaultPageSize){
        if(dt == 1){
            // 设置页码参数（传递repage参数，来记住页码）
            ...
            if(StringUtils.isNumeric(start) && StringUtils.isNumeric(size)){
                CookieUtils.setCookie(response, "start", start);
                CookieUtils.setCookie(response, "length", size);
                String no = ((Integer.parseInt(start)/Integer.parseInt(size))+1)+"";
                if (StringUtils.isNumeric(no)){
                    this.setPageNo(Integer.parseInt(no));
                }else if (request.getParameter("repage")!=null){
                    start = CookieUtils.getCookie(request, "start");
                    size = CookieUtils.getCookie(request, "length");
                    no = ((Integer.parseInt(start)/Integer.parseInt(size))+1)+"";
                    if (StringUtils.isNumeric(no)){
                        this.setPageNo(Integer.parseInt(no));
                    }
                }
            ...
            // 设置排序参数
            Map paraMap = getParametersStartingWith(request,"order[");
            int orderCount = 0 ;
            if(paraMap!=null && paraMap.size()>0){
                orderCount = paraMap.size()/2 ;
            }

            String orderBy = "";
            for(int i=0 ;i<orderCount;i++){
                String orderColIndex = request.getParameter("order["+i+"][column]");
                String orderName = request.getParameter("columns["+orderColIndex+"][name]");
                orderBy += (orderName!=null && !orderName.equals(""))? (orderName + " " + request.getParameter("order["+i+"][dir]") + ",") : "";
            }
            //String orderBy = request.getParameter("orderBy");
            if(orderBy.length()>0){
            ...
            if(StringUtils.isNumeric(start) && StringUtils.isNumeric(size)){
                CookieUtils.setCookie(response, "page", start);
...
//				String no = ((Integer.parseInt(start)/Integer.parseInt(size))+1)+"";
                ...
                if (StringUtils.isNumeric(no)){
                    ...
                    start = CookieUtils.getCookie(request, "page");
                    size = CookieUtils.getCookie(request, "rows");
                    no = ((Integer.parseInt(start)/Integer.parseInt(size))+1)+"";
                    if (StringUtils.isNumeric(no)){
                        this.setPageNo(Integer.parseInt(no));
                    }
            ...
            if(request.getParameter("sort") != null && !"".equals(request.getParameter("sort"))){
                String[] sortArr = request.getParameter("sort").split(",");
                String[] orderArr = request.getParameter("order").split(",");
                for(int i = 0;i < sortArr.length; i++){
                    String sort = sortArr[i];
                    String order = orderArr[i];
                    orderBy += (sort!=null && !sort.equals(""))? (sort + " " + order + ",") : "";
                }
                if(orderBy.length()>0){
                    orderBy = orderBy.substring(0, orderBy.lastIndexOf(","));
                }
